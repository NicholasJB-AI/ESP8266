...
#define CONV_REG                            0x00            /*!< Address For The Conversion Register*/
#define CONFIG_REG                          0x01            /*!< Address For The Configuration Register*/
...
static esp_err_t i2c_example_master_ads1115_init(i2c_port_t i2c_num)
{
    ...
    cmd_data = 0x01;    // Programmable Gain Amplifier Configuration FS = +|- 4.096V Since Vcc = 3.3V
    /*
...
static void i2c_task_example(void *arg)
{
    ...
    */
    ...
    {
        ESP_LOGI(TAG, "|--|--|--|--|--|--|--|\n");
        if(ESP_OK == adc_read(&data[0]))
        {
            ESP_LOGI(TAG, "Voltage Output = %d.%d\n", data[0]/60, data[i]%60);
        }
        if(ESP_OK == adc_read_fast(data, 16))
        {
            for(i = 1; i < 16; i++)
            {
                ESP_LOGI(TAG, "Voltage Output = %d.%d\n", data[i]/60, data[i]%60);
    ...
    AdcConfig.clk_div = 8;
    ESP_ERROR_CHECK(adc_deinit(&AdcConfig));
...
void app_main(void)
{
    ...
    AdcConfig.mode = ADC_READ_TOUT_MODE;
    AdcConfig.clk_div = 8;
    ESP_ERROR_CHECK(adc_init(&AdcConfig));
